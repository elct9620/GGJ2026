...
| 珍珠奶茶 | 3      | 珍珠 ×3                   | 無       | 散射                   | 2s       |
| 豬血糕   | 4      | 米血 ×3                   | 無       | 追蹤彈                 | 2s       |
| 蚵仔煎   | 5      | 擊殺數 ×10                | 無       | 基礎傷害 ×8            | 2s       |

**配方說明**:

- 蚵仔煎為特殊子彈，消耗擊殺數而非食材
- 每次使用消耗 10 個擊殺數（從 Kill Counter 扣除）
- 快吃升級可使蚵仔煎消耗減半（10→5 個擊殺數）
...
| 快吃     | 臭豆腐/珍珠奶茶/豬血糕消耗 -1（當場生效，最低 1）<br>蚵仔煎消耗減半（10→5 個擊殺數） | 無   |
...
- 快吃：當場遊戲生效，臨時減少食材消耗 1（與打折效果可疊加），最低 1；同時使蚵仔煎消耗減半（10→5 個擊殺數，可重複堆疊）
...
- **Consume Kills（消耗擊殺數）**:
  ...
  - 預設消耗：10 個擊殺數
  - 快吃升級效果：消耗減半為 5 個擊殺數（可重複堆疊）
  - 消耗成功後，擊殺數 -10（或 -5），啟動蚵仔煎 Buff

- **Visual Feedback（視覺反饋）**:
  - UI 顯示當前擊殺總數（例：15 隻）
  - UI 顯示蚵仔煎可用狀態（例：10/10 可用 或 5/10 不可用）
  - 遊戲結束時在結算畫面顯示總擊殺數
...
| 操作           | 當前狀態                  | 結果                      |
| -------------- | ------------------------- | ------------------------- |
| 按 5（蚵仔煎） | 擊殺數 < 10               | 無反應，UI 提示擊殺數不足 |
| 按 5（蚵仔煎） | 擊殺數 ≥ 10               | 擊殺數 -10，Buff 啟動 2s  |
| 按 5（蚵仔煎） | 擊殺數 = 15，快吃效果生效 | 擊殺數 -5，Buff 啟動 2s   |

**Integration with Other Systems**:

- **Combat System**: 訂閱 `EnemyDeath` 事件（僅限子彈擊殺）
- **Synthesis System**: 提供擊殺數查詢和消耗介面
- **Upgrade System**: 快吃升級修改蚵仔煎消耗（10→5，可堆疊）
