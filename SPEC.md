...
- **Consume Kills（消耗擊殺數）**:
  ...
  - 預設消耗：每次使用消耗固定 20 個擊殺數
  - 消耗成功後，擊殺數 -20，發射蚵仔煎子彈（單發）
  - 可重複使用：只要擊殺數 ≥20 即可再次觸發
  - 多次消耗範例：
    - 42 擊殺數 → 按鍵 5（第 1 次）→ 消耗 20 → 剩餘 22
    - 22 擊殺數 → 按鍵 5（第 2 次）→ 消耗 20 → 剩餘 2
    - 2 擊殺數 → 按鍵 5（第 3 次）→ 無反應（不足 20）
...
| 單次消耗（有餘）| 25     | 5 成功 | 擊殺數 -20 → 剩餘 5           |
| 多次消耗（第1次）| 42     | 5 成功 | 擊殺數 -20 → 剩餘 22          |
| 多次消耗（第2次）| 22     | 5 成功 | 擊殺數 -20 → 剩餘 2           |
| 多次消耗（不足）| 2      | 5      | 無反應（2<20）                |
| 快吃加成       | 任何   | 5 成功 | 傷害 +10%（消耗仍為 20）      |
