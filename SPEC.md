...
- 直接從攤位消耗食材
- 特殊子彈分為兩類：
  - **限時 Buff（1-4 號）**：啟動 2 秒 Buff，Buff 期間無法再次觸發
  - **即時施放（5 號蚵仔煎）**：立即發射一發子彈，不佔用 Buff 狀態
- 按鍵觸發採用**邊緣檢測（Edge Detection）**：只在按鍵從未按下變為按下時觸發一次

**Behaviors**:

- **Trigger by Key（按鍵觸發）**:
  - 偵測按鍵從未按下變為按下的瞬間（邊緣觸發）
  - 檢查食材或擊殺數是否滿足配方
  - 食材充足時立即消耗並啟動對應效果
...
- **Activate Buff（啟動限時 Buff，1-4 號）**:
  - 接下來發射的子彈擁有特殊效果（2 秒內）
  - UI 顯示當前 Buff 類型和剩餘時間
  - Buff 期間無法觸發其他特殊子彈（包含蚵仔煎）

- **Instant Cast（即時施放，5 號蚵仔煎）**:
  - 立即發射一發百分比傷害子彈（不啟動 Buff）
  - 發射後不佔用 Buff 狀態（立即恢復可射擊）
  - Buff 期間無法觸發（受限時 Buff 堵塞影響）
  - 傷害計算：讀取 UpgradeSystem 的快吃加成（+10% 百分比，直接相加）

- **Buff Expiration（Buff 結束，1-4 號）**:
  - 2 秒後 Buff 結束
  - 恢復普通子彈
...
- **蚵仔煎（5 號）**：
  - 消耗擊殺數而非食材（每次 20 個擊殺數）
  - **即時施放**：按下按鍵立即發射一發百分比傷害子彈（不啟動 Buff）
  - 百分比傷害（基礎值）：Boss 10% 當前 HP，菁英怪 50% 當前 HP，小怪 70% 當前 HP
  - **快吃升級**：百分比直接相加（Boss 10%→20%，菁英 50%→60%，小怪 70%→80%）
  - **Buff 堵塞影響**：Buff 期間無法觸發（遊戲平衡設計，避免 Buff+大招疊加過強）
- **其他特殊子彈（1-4 號）**：
  - 消耗食材（豆腐/珍珠/米血）
  - **限時 Buff**：啟動 2 秒 Buff，期間發射的子彈擁有特殊效果
  - Buff 期間無法再次觸發任何特殊子彈（包含蚵仔煎）
- **邊緣觸發**：所有按鍵只在從未按下變為按下時觸發一次，避免按住按鍵連續觸發

**Error Scenarios Decision Table**:

| 珍珠   | 豆腐 | 米血 | 擊殺 | Buff | 按鍵 | 邊緣觸發 | 結果               |
| ------ | ---- | ---- | ---- | ---- | ---- | -------- | ------------------ |
| ≥1     | ≥1   | ≥1   | 任何 | 無   | 1    | 是       | 夜市總匯成功       |
| 任一<1 | -    | -    | 任何 | 無   | 1    | 是       | 失敗，提示食材不足 |
| 任何   | ≥3   | 任何 | 任何 | 無   | 2    | 是       | 臭豆腐成功         |
| 任何   | <3   | 任何 | 任何 | 無   | 2    | 是       | 失敗，提示食材不足 |
| ≥3     | 任何 | 任何 | 任何 | 無   | 3    | 是       | 珍珠奶茶成功       |
| <3     | 任何 | 任何 | 任何 | 無   | 3    | 是       | 失敗，提示食材不足 |
| 任何   | 任何 | ≥3   | 任何 | 無   | 4    | 是       | 豬血糕成功         |
| 任何   | 任何 | <3   | 任何 | 無   | 4    | 是       | 失敗，提示食材不足 |
| 任何   | 任何 | 任何 | ≥20  | 無   | 5    | 是       | 蚵仔煎成功（即時） |
| 任何   | 任何 | 任何 | <20  | 任何 | 5    | 是       | 失敗，提示擊殺不足 |
| 任何   | 任何 | 任何 | ≥20  | 有   | 5    | 是       | 無效果（Buff 中）  |
| 任何   | 任何 | 任何 | 任何 | 有   | 1-4  | 是       | 無效果（Buff 中）  |
| 任何   | 任何 | 任何 | 任何 | 任何 | 1-5  | 否       | 無效果（非邊緣）   |
...
| 按鍵  | 功能     | 行為                                                       | 觸發方式 |
| ----- | -------- | ---------------------------------------------------------- | -------- |
| W     | 向上移動 | 玩家向上移動（200 px/s）                                   | 按住持續 |
| A     | 向左移動 | 玩家向左移動（200 px/s）                                   | 按住持續 |
| S     | 向下移動 | 玩家向下移動（200 px/s）                                   | 按住持續 |
| D     | 向右移動 | 玩家向右移動（200 px/s）                                   | 按住持續 |
| Space | 射擊     | 發射子彈（消耗彈夾）                                       | 邊緣觸發 |
| 1     | 夜市總匯 | 消耗豆腐1、珍珠1、米血1，啟動連鎖閃電 Buff（2s）           | 邊緣觸發 |
| 2     | 臭豆腐   | 消耗豆腐3，啟動貫穿 Buff（2s）                             | 邊緣觸發 |
| 3     | 珍珠奶茶 | 消耗珍珠3，啟動散射 Buff（2s）                             | 邊緣觸發 |
| 4     | 豬血糕   | 消耗米血3，啟動追蹤 Buff（2s）                             | 邊緣觸發 |
| 5     | 蚵仔煎   | 消耗擊殺數20個，**立即發射**一發百分比傷害子彈（不啟動 Buff） | 邊緣觸發 |

**觸發方式說明**:

- **按住持續（Hold）**：按鍵被按住期間持續執行動作（移動）
- **邊緣觸發（Edge Detection）**：只在按鍵從未按下變為按下的瞬間觸發一次，避免按住按鍵連續觸發

**Input Controls Decision Table**:

| 輸入  | 遊戲狀態 | 彈藥 | Buff   | 邊緣觸發 | 邊界 | 結果              |
| ----- | -------- | ---- | ------ | -------- | ---- | ----------------- |
| Space | 進行中   | >0   | 任何   | 是       | -    | 發射              |
| Space | 進行中   | >0   | 任何   | 否       | -    | 無效果（已發射）  |
| Space | 進行中   | 0    | 任何   | 任何     | -    | 無效果            |
| Space | 重裝中   | 任何 | 任何   | 任何     | -    | 無效果            |
| 1-4   | 進行中   | 任何 | 無     | 是       | -    | 合成嘗試（Buff）  |
| 1-4   | 進行中   | 任何 | 無     | 否       | -    | 無效果（非邊緣）  |
| 1-4   | 進行中   | 任何 | 有     | 任何     | -    | 無效果（Buff 中） |
| 5     | 進行中   | 任何 | 任何   | 是       | -    | 合成嘗試（即時）  |
| 5     | 進行中   | 任何 | 任何   | 否       | -    | 無效果（非邊緣）  |
| WASD  | 進行中   | 任何 | 任何   | 任何     | 內部 | 移動 200px/s      |
| WASD  | 進行中   | 任何 | 任何   | 任何     | 邊緣 | 停在邊界          |
| WASD  | 重裝中   | 任何 | 任何   | 任何     | 任何 | 正常移動          |
...
**設計重點**: 透過「辨識度」與「打擊感」讓玩家直觀感受子彈特性（如：穿透、連鎖、緩速）。
...
| 狀態     | 視覺效果                        | 設計意圖               |
| -------- | ------------------------------- | ---------------------- |
| 飛行中   | 尾端帶有一點點白色的風切線條    | 表示子彈被射出         |
| 命中敵人 | 產生白色「點狀」撞擊特效（Pop） | 簡潔，不搶特殊子彈風采 |
...
| 狀態     | 視覺效果                                                 | 設計意圖                           |
| -------- | -------------------------------------------------------- | ---------------------------------- |
| 飛行中   | 子彈周圍環繞著金黃色的電流                               | 視覺化「連鎖」概念                 |
| 命中敵人 | 產生金黃色閃電鏈，折射跳躍至後方最近的 4 個敵人          | 用「電流」或「醬汁鏈」呈現連鎖效果 |
| 連鎖目標 | 被鏈結的敵人身上短暫閃爍金光                             | 強調高級感                         |
| 子彈行為 | 命中第一個敵人後不消失，持續連鎖直到目標數達上限或無敵人 | 連鎖機制視覺化                     |
...
| 狀態     | 視覺效果                                             | 設計意圖                   |
| -------- | ---------------------------------------------------- | -------------------------- |
| 飛行中   | 豆腐後方拖著兩條波浪狀的綠色氣體尾跡，飛行速度較快   | 視覺化「臭味」，帶有衝擊感 |
| 貫穿敵人 | 穿過第一個敵人時不停下，在該敵人背後爆出一團綠色臭氣 | 綠色氣體強調「貫穿」力度   |
...
| 狀態   | 視覺效果                           | 設計意圖                       |
| ------ | ---------------------------------- | ------------------------------ |
| 發射時 | 呈扇形射出三顆巨大的子彈（非單顆） | 圓潤黑珍珠對比方形豆腐與米血   |
| 飛行中 | 三顆子彈分別沿散射軌跡飛行         | 散射軌跡覆蓋範圍大，視覺滿足感 |
...
| 狀態   | 視覺效果                                               | 設計意圖                           |
| ------ | ------------------------------------------------------ | ---------------------------------- |
| 飛行中 | 帶有曲線或螺旋軌跡飛向最近的敵人，尾部拖著黏稠黑色殘影 | 強調「黏膩感」呼應緩速效果         |
| 軌跡   | 非直線飛行，表現「追蹤」的靈性                         | 花生粉黃色與黑色糕體對比，易於辨識 |
...
| 狀態   | 視覺效果                                      | 設計意圖                             |
| ------ | --------------------------------------------- | ------------------------------------ |
| 飛行中 | 巨大的拋物線投擲物（體積比普通子彈大 3-4 倍） | 作為大招必須有「重量感」和「毀滅性」 |
| 命中時 | 引發全螢幕震動（Screen Shake）                | 紅色甜辣醬爆炸視覺張力               |
...
| 子彈類型 | 粒子效果                | 動畫需求             | 技術實作                                 |
| -------- | ----------------------- | -------------------- | ---------------------------------------- |
| 普通子彈 | 白色風切線條 + Pop 特效 | 尾部拖尾，命中爆炸   | 簡單粒子發射器                           |
| 夜市總匯 | 金黃色電流環繞 + 閃電鏈 | 連鎖跳躍動畫         | 多目標連線動畫，目標閃爍效果             |
| 臭豆腐   | 綠色氣體尾跡 + 貫穿臭氣 | 波浪狀尾部，貫穿爆炸 | 持續性粒子發射，貫穿不消失               |
| 珍珠奶茶 | 三向散射                | 扇形發射             | 多子彈生成，不同角度                     |
| 豬血糕   | 黑色黏稠殘影            | 曲線/螺旋軌跡        | 追蹤算法（Vector 計算），拖尾粒子        |
| 蚵仔煎   | 拋物線投擲 + 螢幕震動   | 大型爆炸特效         | 重力拋物線，螢幕震動效果（Screen Shake） |

**實作優先級**:

1. **Phase 1（MVP）**：普通子彈基礎視覺（風切線條 + Pop）
2. **Phase 2（特殊效果）**：特殊子彈粒子效果（電流、氣體、散射、殘影）
3. **Phase 3（進階效果）**：連鎖動畫、螢幕震動、追蹤軌跡
