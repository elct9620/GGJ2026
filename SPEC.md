...
| **Box（寶箱）**              | 出現在攤位前方的視覺防線標記，顯示有食材儲存，敵人碰撞時扣玩家生命並消除敵人 |
...
**Purpose**: 視覺化防線標記，顯示攤位食材儲存狀態，敵人碰撞時扣玩家生命
...
| 操作         | 當前狀態          | 結果                                   |
| ------------ | ----------------- | -------------------------------------- |
| 敵人碰撞寶箱 | 食材 1/6          | 玩家生命 -1，敵人消失不掉落，食材不變  |
| 敵人進攻     | 寶箱不存在（0/6） | 敵人進入攤位區，正常偷取食材           |
| 攤位補充食材 | 寶箱已消失        | 新寶箱立即出現                         |
| 玩家觸發合成 | 寶箱存在          | 食材消耗，寶箱可能消失（食材歸零時）   |
...
| 食材總數 > 0 | 寶箱出現 | 寶箱可見                       |
| 食材總數 = 0 | 寶箱消失 | 無寶箱                         |
| 敵人碰撞寶箱 | 扣血     | 玩家生命 -1, 敵人消失（不掉落）|

**Integration with Other Systems**:

- **Booth System**: 寶箱存在狀態鏡像攤位食材數量（食材 > 0 時存在）
- **Player**: 敵人碰撞寶箱時扣玩家生命值（與底線相同）
- **EventQueue**: 訂閱 `FoodStored` 和 `FoodConsumed` 事件更新顯示狀態
