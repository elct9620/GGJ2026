...
- 無限回合（直到玩家死亡，無勝利條件）
- 敵人數量線性增長：回合數 × 3（第 1 回合 3 隻，第 2 回合 6 隻，依此類推）
...
- **Consume Kills（消耗擊殺數）**:
  - 按鍵 5 觸發蚵仔煎時，檢查擊殺數是否足夠（≥20）
  - 每次消耗：20 個擊殺數（固定值）
  - 消耗成功後：擊殺數 -20，啟動蚵仔煎 Buff（單發）
  - **多次消耗機制**：累積擊殺數為 20n+k 時，可使用 m 次蚵仔煎，剩餘擊殺數 = (20n+k) - 20m
    - 範例 1：25 隻擊殺 → 使用 1 次 → 剩餘 5 隻（25 - 20 = 5）
    - 範例 2：40 隻擊殺 → 使用 2 次 → 剩餘 0 隻（40 - 20 - 20 = 0）
    - 範例 3：48 隻擊殺 → 使用 2 次 → 剩餘 8 隻（48 - 40 = 8，第 3 次無法使用）
...
| 解鎖條件     | ≥20    | 5      | 可觸發                   |
| 擊殺不足     | <20    | 5      | 無反應，UI 提示          |
| 單次消耗     | 20     | 5 成功 | 擊殺數 -20，剩餘 0       |
| 單次消耗     | 25     | 5 成功 | 擊殺數 -20，剩餘 5       |
| 多次消耗     | 40     | 5 兩次 | 第一次剩 20，第二次剩 0  |
| 多次消耗     | 48     | 5 兩次 | 第一次剩 28，第二次剩 8  |
| 消耗後不足   | 8      | 5      | 無反應（8<20）           |
| 快吃加成     | 任何   | 5 成功 | 傷害 +10%（不影響消耗）  |
...
| 名稱   | 類型        | 敵人分類 | 基礎 HP | 速度 (px/s) | 掉落物   | 行為 | HP 成長公式             |
| ------ | ----------- | -------- | ------- | ----------- | -------- | ---- | ----------------------- |
| 餓鬼   | Ghost       | 小怪     | 1       | 160         | **無**   | 直走 | floor(1 + (W-1) × 0.03) |
| 紅餓鬼 | Red Ghost   | 菁英     | 2       | 130         | 豆腐     | 直走 | round(2 + (W-1) × 0.6)  |
| 綠餓鬼 | Green Ghost | 菁英     | 2       | 130         | 珍珠     | 直走 | round(2 + (W-1) × 0.6)  |
| 藍餓鬼 | Blue Ghost  | 菁英     | 2       | 130         | 米血     | 直走 | round(2 + (W-1) × 0.6)  |
| 餓死鬼 | Boss        | Boss     | 10      | 100         | 特殊升級 | 直走 | round(10 + (W-5) × 1.5) |
| 餓瘋鬼 | Boss        | Boss     | 10      | 100         | 特殊升級 | 斜走 | round(10 + (W-5) × 1.5) |
| 餓幼鬼 | Boss        | Boss     | 6       | 210         | 特殊升級 | 直走 | round(6 + (W-5) × 1.0)  |
...
- 向左移動
- 碰撞寶箱時消耗對應攤位食材，敵人立即消失（不掉落食材）
- 寶箱不存在時穿透到底線（x = 340），扣玩家生命並消失
- 被子彈擊中扣除生命值
- 生命值歸零時死亡並掉落食材
...
- 三欄佈局（總寬 1920 px）：
  - 左側（550 px）：升級圖示列表
  - 中央（820 px）：技能按鈕（1-5 數字鍵對應，顯示可用/不可用狀態）
  - 右側（550 px）：操作說明
...
- 玩家生命值和彈藥數移至玩家隨行資訊（§ 2.7.4）

### 2.7.4 Player Floating Info

**Purpose**: 在玩家角色附近顯示即時戰鬥狀態（圖示化），讓玩家無需看 HUD 即可掌握關鍵數值。

**結構**:

```
PlayerRenderer Container
├── playerSprite (256×256)
├── dirHintSprite (100×256)
├── 生命值圖示列 (玩家正下方)
└── 彈藥進度條 (生命值正下方)
```
...
| ------ | ---------- | ------------------------------------ |
| 滿彈   | [████████] | 進度條完全填滿                       |
| 射擊後 | [████░░░░] | 比例呈現剩餘/最大（如 3/6 = 50%）    |
| 重裝中 | [動畫填充] | 進度條動畫，3 秒內由當前值填滿至滿彈 |

**大胃王升級支援**:

彈匣容量可透過「大胃王」升級增加（6→12→18→24）：
...
- 隨玩家移動（跟隨 PlayerRenderer Container）
- 進度條尺寸固定，僅比例動態調整
- 重裝動畫需平滑過渡（3 秒內完成填充）
- 不遮蓋玩家角色主體
- 圖示需具備高辨識度（即使在戰鬥混亂中也能快速讀取）
...
| 敵人   | 寶箱   | AABB           | 食材 -1，敵人消失（不掉落） |
| 敵人   | 底線   | X 座標檢查     | 扣玩家生命（寶箱不存在時）  |
| 玩家   | 敵人   | 無碰撞（穿透） | 無效果                      |
| 玩家   | 邊界   | 邊界座標限制   | 停止移動                    |
