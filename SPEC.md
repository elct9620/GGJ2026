...
- 視覺順序與攤位編號系統獨立：攤位編號（Booth 1=珍珠, Booth 2=豆腐, Booth 3=米血）保持不變
- 按鍵綁定不受視覺順序影響：按鍵 1=珍珠奶茶（消耗珍珠×3），按鍵 2=臭豆腐（消耗豆腐×3），按鍵 3=豬血糕（消耗米血×3）
- 垂直間距：攤位高度 256px + 間距 11px = 267px（每個攤位間隔）
