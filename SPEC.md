...
| **Audio System（音效系統）** | 管理背景音樂和音效播放的系統，透過事件驅動觸發音效                         |
| **Sound Effect（音效）**     | 遊戲中的短促音訊反饋，如射擊、受擊、按鈕點擊等                             |
| **Background Music（背景音樂）** | 遊戲場景中循環播放的背景音訊                                         |
...
| `PlayerShoot`        | `{}`                                    | 玩家發射子彈                 | Audio System                              |
| `EnemyHit`           | `{ enemyId: string }`                   | 子彈擊中敵人                 | Audio System                              |
| `ButtonClick`        | `{ buttonId: string }`                  | UI 按鈕點擊                  | Audio System                              |
| `GameSceneEnter`     | `{}`                                    | 遊戲場景進入                 | Audio System                              |
...
- **Audio System**:
  - 訂閱 `PlayerShoot` 事件（播放射擊音效）
  - 訂閱 `EnemyHit` 事件（播放受擊音效）
  - 訂閱 `ButtonClick` 事件（播放按鈕音效）
  - 訂閱 `SynthesisTriggered` 事件（播放合成音效）
  - 訂閱 `UpgradeSelected` 事件（播放升級音效）
  - 訂閱 `GameSceneEnter` 事件（開始背景音樂）
...
- 所有音效檔案位於 `src/assets/se/` 目錄

**Behaviors**:

- **Background Music（背景音樂）**:
  - 遊戲場景進入時自動播放 `Leisure song.mp3`
  - 循環播放（loop = true）
  - 音量：0.5（可調整）

- **Sound Effects（音效）**:
  - 按鈕點擊：播放 `select03.mp3`（任何按鈕 onClick、合成成功、升級選擇）
  - 玩家射擊：播放 `shoot5.mp3`（玩家發射子彈時）
  - 敵人受擊：播放 `short_punch1.mp3`（子彈擊中敵人時）

**Audio Trigger Decision Table**:

| 事件類型            | 觸發條件           | 音效檔案           | 重疊播放 |
| ------------------- | ------------------ | ------------------ | -------- |
| ButtonClick         | 按鈕點擊           | select03.mp3       | 是       |
| PlayerShoot         | Space 鍵發射子彈   | shoot5.mp3         | 是       |
| EnemyHit            | 子彈擊中敵人       | short_punch1.mp3   | 是       |
| SynthesisSuccess    | 合成成功           | select03.mp3       | 是       |
| UpgradeSelected     | 升級選擇           | select03.mp3       | 是       |
| BackgroundMusicLoop | 遊戲場景進入       | Leisure song.mp3   | 否       |

**Error Scenarios**:

| 操作         | 當前狀態       | 結果                       |
| ------------ | -------------- | -------------------------- |
| 播放音效     | 音效檔案不存在 | 記錄錯誤，繼續遊戲         |
| 播放背景音樂 | 音效檔案不存在 | 記錄錯誤，繼續遊戲         |
| 重複播放音樂 | 音樂已播放     | 忽略（避免重複播放）       |
| 停止音樂     | 音樂未播放     | 無效果                     |

**Integration with Systems**:

- **EventQueue**: 訂閱以下事件以觸發音效
  - `PlayerShoot`: 發射子彈音效
  - `EnemyHit`: 敵人受擊音效（新增事件類型）
  - `ButtonClick`: 按鈕點擊音效（新增事件類型）
  - `SynthesisTriggered`: 合成成功音效
  - `UpgradeSelected`: 升級選擇音效
  - `GameSceneEnter`: 背景音樂開始（新增事件類型）

- **Combat System**: 發佈 `PlayerShoot` 和 `EnemyHit` 事件
- **Synthesis System**: 發佈 `SynthesisTriggered` 事件時觸發音效
- **Upgrade System**: 發佈 `UpgradeSelected` 事件時觸發音效
- **UI System**: 按鈕點擊時發佈 `ButtonClick` 事件

**Audio File Mapping**:

| 音效名稱     | 檔案路徑                       | 用途         | 格式 | 大小限制 |
| ------------ | ------------------------------ | ------------ | ---- | -------- |
| 背景音樂     | src/assets/se/Leisure song.mp3 | 遊戲場景背景 | MP3  | < 2 MB   |
| 按鈕點擊音效 | src/assets/se/select03.mp3     | UI 互動反饋  | MP3  | < 100 KB |
| 射擊音效     | src/assets/se/shoot5.mp3       | 玩家射擊     | MP3  | < 100 KB |
| 受擊音效     | src/assets/se/short_punch1.mp3 | 敵人受擊     | MP3  | < 100 KB |
