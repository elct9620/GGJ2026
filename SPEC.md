...
| **Kill Counter（擊殺計數）** | 追踪玩家擊殺敵人數量的系統，用於解鎖蚵仔煎特殊子彈                         |
...
| 按鍵         | 當前狀態               | 結果                    |
| ------------ | ---------------------- | ----------------------- |
| 1-5          | 食材不足               | 無反應，UI 提示庫存不足 |
| 1-5          | Buff 期間              | 無反應（無法疊加）      |
| 1 (夜市總匯) | 豆腐/珍珠/米血任一為 0 | 無反應                  |
| 5 (蚵仔煎)   | 未達擊殺門檻           | 無反應，UI 提示未解鎖   |
...
| 升級名稱 | 效果                                     | 消耗 |
| -------- | ---------------------------------------- | ---- |
| 打折     | 臭豆腐/珍珠奶茶/豬血糕消耗 -1（3→2）     | 無   |
| 大胃王   | 彈匣容量 ×2（6→12→24...）                | 無   |
| 快吃     | 蚵仔煎擊殺門檻 ÷2（10→5→2...，向下取整） | 無   |
| 飢餓三十 | 特殊子彈 Buff 時間 ×2（2s→4s→8s...）     | 無   |

**升級效果說明**:

- 打折：僅影響 3 種基礎特殊子彈，不影響夜市總匯和蚵仔煎
- 大胃王：每次選擇使彈匣容量翻倍，可重複堆疊
- 快吃：降低解鎖門檻，已解鎖玩家無影響（僅影響新遊戲）
...
| `KillCounterUnlocked` | `{}`                                    | 擊殺計數達到 10              | UI System, Synthesis System               |
...
| 操作     | 當前狀態      | 結果                                |
| -------- | ------------- | ----------------------------------- |
| 按鍵 5   | 計數器 < 10   | 無反應，UI 提示「需擊殺 10 隻敵人」 |
| 按鍵 5   | 計數器 ≥ 10   | 正常觸發蚵仔煎 Buff（2 秒）         |
| 擊殺敵人 | 計數器已達 10 | 計數器繼續累積（無上限）            |

**Integration with Other Systems**:

- **Combat System**: 訂閱 `EnemyDeath` 事件（僅限子彈擊殺）
- **UI System**: 顯示擊殺進度和解鎖狀態
- **EventQueue**: 發佈 `KillCounterUnlocked` 事件（達到門檻時）
...
| 5     | 蚵仔煎   | 擊殺 ≥ 10 隻後解鎖，發射高傷害子彈（2s，無消耗） |
...
| -------- | ------------ | ----------------------- |
| 按 Space | 重裝期間     | 無反應                  |
| 按 1-5   | 食材不足     | 無反應，UI 提示庫存不足 |
| 按 1-5   | Buff 期間    | 無反應（無法疊加）      |
| 按 5     | 未達擊殺門檻 | 無反應，UI 提示未解鎖   |
| 移動鍵   | 超出遊戲邊界 | 停止移動                |
