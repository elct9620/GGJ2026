...
describe("Player", () => {
  ...
  describe("Shooting", () => {
    ...
    it("彈夾空時無法射擊", () => {
      // Shoot all 8 bullets, last one triggers auto-reload
      for (let i = 0; i < 8; i++) player.shoot();
    ...
    it("彈夾歸零時自動開始重裝", () => {
      // Shoot 7 bullets to have 1 remaining
      for (let i = 0; i < 7; i++) player.shoot();
    ...
    it("重裝時間為 2.5 秒", () => {
      expect(player.reloadTime).toBe(2.5);
    });
  ...
  describe("Reset", () => {
    it("reset 恢復所有狀態", () => {
      // Modify player state
      player.takeDamage(3);
      // Shoot 4 bullets to have 4 remaining
  ...
  // Note: Appearance management moved to PlayerRenderer (Phase 2 architecture refactor)
  // See src/renderers/player-renderer.test.ts for buff appearance tests
});
