...
## 2.12 Audio System Tests

### 2.12.1 Background Music

| Test ID | Input                | Expected Output           | Accept Criteria                |
| ------- | -------------------- | ------------------------- | ------------------------------ |
| AU-01   | 遊戲場景進入         | 背景音樂開始播放          | 播放 Leisure song.mp3          |
| AU-02   | 背景音樂播放         | 循環播放                  | loop = true，音樂不停止        |
| AU-03   | 遊戲暫停             | 背景音樂繼續播放          | 音樂不受遊戲狀態影響           |
| AU-04   | 遊戲結束             | 背景音樂繼續播放          | 音樂不停止                     |
| AU-05   | 音樂檔案載入失敗     | 遊戲繼續，無背景音樂      | 記錄錯誤，不阻塞遊戲           |
| AU-06   | 瀏覽器限制自動播放   | 使用者互動後播放          | 第一次點擊後音樂開始           |

### 2.12.2 Sound Effects - Button Click

| Test ID | Input                | Expected Output           | Accept Criteria                |
| ------- | -------------------- | ------------------------- | ------------------------------ |
| AU-07   | 點擊開始遊戲按鈕     | 播放按鈕音效              | 播放 select03.mp3              |
| AU-08   | 點擊升級選項         | 播放按鈕音效              | 播放 select03.mp3              |
| AU-09   | 按鍵 1-5 合成成功    | 播放按鈕音效              | 播放 select03.mp3              |
| AU-10   | 短時間內連續點擊按鈕 | 音效重疊播放              | 每次點擊觸發新的音效實例       |
| AU-11   | 按鈕音效檔案載入失敗 | 遊戲繼續，無音效          | 忽略錯誤，不阻塞遊戲           |

### 2.12.3 Sound Effects - Player Shoot

| Test ID | Input                | Expected Output           | Accept Criteria                |
| ------- | -------------------- | ------------------------- | ------------------------------ |
| AU-12   | 按 Space 發射子彈    | 播放射擊音效              | 播放 shoot5.mp3                |
| AU-13   | 彈夾歸零，無法射擊   | 不播放射擊音效            | 無音效觸發                     |
| AU-14   | 重裝期間按 Space     | 不播放射擊音效            | 無音效觸發                     |
| AU-15   | 快速連續射擊（6 發） | 音效重疊播放              | 每次射擊觸發新的音效實例       |
| AU-16   | 射擊音效檔案載入失敗 | 遊戲繼續，無音效          | 忽略錯誤，不阻塞遊戲           |

### 2.12.4 Sound Effects - Enemy Hit

| Test ID | Input                   | Expected Output           | Accept Criteria                |
| ------- | ----------------------- | ------------------------- | ------------------------------ |
| AU-17   | 子彈擊中敵人            | 播放擊中音效              | 播放 short_punch1.mp3          |
| AU-18   | 子彈同時擊中多隻敵人    | 音效重疊播放              | 每次擊中觸發新的音效實例       |
| AU-19   | 散射子彈擊中 3 隻敵人   | 播放 3 次擊中音效         | 3 個音效實例重疊播放           |
| AU-20   | 連鎖子彈擊中 5 隻敵人   | 播放 5 次擊中音效         | 5 個音效實例依序觸發           |
| AU-21   | 敵人到達底線（未擊中）  | 不播放擊中音效            | 無音效觸發                     |
| AU-22   | 擊中音效檔案載入失敗    | 遊戲繼續，無音效          | 忽略錯誤，不阻塞遊戲           |

### 2.12.5 Event Integration

| Test ID | Input                   | Expected Output                      | Accept Criteria                |
| ------- | ----------------------- | ------------------------------------ | ------------------------------ |
| AU-23   | 發佈 SoundEffectTriggered 事件 | Audio System 接收並播放音效   | 事件訂閱正常                   |
| AU-24   | soundId = "button"      | 播放 select03.mp3                    | 音效映射正確                   |
| AU-25   | soundId = "shoot"       | 播放 shoot5.mp3                      | 音效映射正確                   |
| AU-26   | soundId = "hit"         | 播放 short_punch1.mp3                | 音效映射正確                   |
| AU-27   | soundId = "invalid"     | 記錄錯誤，忽略播放                   | 錯誤處理正確                   |
| AU-28   | 發佈 BackgroundMusicStart 事件 | Audio System 播放背景音樂        | 事件訂閱正常                   |
...
| IT-12   | 音效系統 | 戰鬥系統 | 子彈發射觸發音效   | 播放射擊音效             | 事件通訊正常    |
| IT-13   | 音效系統 | 戰鬥系統 | 敵人受傷觸發音效   | 播放擊中音效             | 事件通訊正常    |
| IT-14   | 音效系統 | 合成系統 | 合成成功觸發音效   | 播放按鈕音效             | 事件通訊正常    |
| IT-15   | 音效系統 | 升級系統 | 升級選擇觸發音效   | 播放按鈕音效             | 事件通訊正常    |
| IT-16   | 音效系統 | 遊戲場景 | 場景初始化播放音樂 | 背景音樂循環播放         | 初始化流程正常  |
