...
**測試目標**: 驗證子彈視覺效果符合 SPEC.md § 2.6.3 規格，確保「辨識度」與「打擊感」。

| Test ID | Bullet Type | Visual Element | Expected Behavior                     | Accept Criteria                         |
| ------- | ----------- | -------------- | ------------------------------------- | --------------------------------------- |
| VE-01   | 普通子彈    | 飛行尾跡       | 尾端帶有白色風切線條                  | 視覺可見，不過於顯眼                    |
| VE-02   | 普通子彈    | 命中特效       | 擊中敵人產生白色點狀 Pop 特效         | 簡潔清晰，不遮蓋遊戲畫面                |
| VE-03   | 夜市總匯    | 飛行特效       | 子彈周圍環繞金黃色電流                | 電流動畫流暢，顏色為金黃色              |
| VE-04   | 夜市總匯    | 連鎖特效       | 命中後產生金黃色閃電鏈跳躍至 4 個敵人 | 閃電鏈視覺清晰，跳躍動畫流暢            |
| VE-05   | 夜市總匯    | 目標閃爍       | 被鏈結的敵人短暫閃爍金光              | 閃爍效果明顯但不刺眼                    |
| VE-06   | 臭豆腐      | 飛行尾跡       | 豆腐後方拖著兩條波浪狀綠色氣體尾跡    | 綠色氣體波浪動畫，易於辨識              |
| VE-07   | 臭豆腐      | 貫穿特效       | 穿過敵人時在其背後爆出綠色臭氣        | 貫穿動畫不停頓，臭氣特效明顯            |
| VE-08   | 珍珠奶茶    | 散射發射       | 發射瞬間呈扇形射出三顆子彈            | 三顆子彈同時生成，扇形角度合理          |
| VE-09   | 珍珠奶茶    | 子彈大小       | 子彈視覺上較普通子彈巨大              | 視覺尺寸明顯大於普通子彈                |
| VE-10   | 豬血糕      | 追蹤軌跡       | 帶有曲線或螺旋軌跡飛向最近敵人        | 軌跡非直線，追蹤行為明顯                |
| VE-11   | 豬血糕      | 黏稠殘影       | 尾部拖著黑色黏稠殘影                  | 黑色殘影持續時間適當，呈現黏膩感        |
| VE-12   | 蚵仔煎      | 拋物線軌跡     | 巨大投擲物沿拋物線飛行                | 拋物線動畫流暢，體積比普通子彈大 3-4 倍 |
| VE-13   | 蚵仔煎      | 螢幕震動       | 命中時引發全螢幕震動（Screen Shake）  | 震動效果明顯但不過度，持續時間適當      |
| VE-14   | 所有特殊彈  | 辨識度         | 不同子彈類型視覺差異明顯              | 玩家可快速識別當前子彈類型              |
| VE-15   | 所有子彈    | 打擊感         | 命中敵人時視覺反饋清晰                | 命中感明確，反饋即時                    |

**視覺效果測試方法**:

- **單元測試層級**：驗證粒子系統初始化、參數配置正確
- **整合測試層級**：驗證視覺效果與遊戲邏輯同步（如：連鎖特效與連鎖傷害同時發生）
- **手動測試層級**：玩家視角驗證視覺辨識度、打擊感、流暢度

**視覺效果品質標準**:

| 評估項目 | 標準                                 |
| -------- | ------------------------------------ |
| 辨識度   | 5 種特殊子彈視覺差異明顯，不易混淆   |
| 打擊感   | 命中特效清晰有力，即時反饋           |
| 效能影響 | 視覺效果不影響 FPS（維持 ≥55 FPS）   |
| 視覺干擾 | 特效不遮蓋重要遊戲資訊（敵人、玩家） |
...
### 2.7.6 Booth Interaction

| Test ID | Input               | Expected Output  | Accept Criteria |
| ------- | ------------------- | ---------------- | --------------- |
| EN-36   | 攤位 3/6 + 敵人觸碰 | 攤位 2/6         | 偷取 1 個食材   |
| EN-37   | 敵人偷取後          | 敵人繼續向左移動 | 不停留在攤位    |
| EN-38   | 攤位 0/6 + 敵人觸碰 | 攤位 0/6         | 無食材可偷      |
...
## 2.10.5 Bullet Visual Effects Tests

**測試範圍**: 參見 SPEC.md § 2.6.3 Bullets 視覺效果規格

**測試策略**: 子彈視覺效果主要透過手動視覺測試（Manual Visual Testing）驗證，單元測試僅驗證視覺效果系統的初始化和配置正確性。完整的粒子效果、光影、震動需在瀏覽器中實際觀察。

### 2.10.5.1 Visual Effects System Initialization

| Test ID | Input                        | Expected Output                  | Accept Criteria                      |
| ------- | ---------------------------- | -------------------------------- | ------------------------------------ |
| VE-01   | 建立 BulletVisualEffects     | 系統初始化成功                   | 無錯誤拋出，粒子池預分配完成         |
| VE-02   | 普通子彈發射                 | 產生拖尾粒子配置                 | 粒子數 2-3/幀，壽命 0.2s             |
| VE-03   | 普通子彈命中敵人             | 產生爆發粒子配置                 | 粒子數 30-50，擴散角 360°            |
| VE-04   | 夜市總匯發射                 | 產生電弧環繞配置                 | 電弧數 8-12，旋轉速度 180°/s         |
| VE-05   | 夜市總匯命中 + 連鎖 5 目標   | 產生連鎖線條配置                 | 線條數 5，每段衰減 -20% 亮度         |
| VE-06   | 臭豆腐發射                   | 產生波浪尾跡配置                 | 粒子數 60-80，壽命 0.6s              |
| VE-07   | 臭豆腐貫穿敵人               | 產生貫穿爆發配置                 | 粒子數 100-150，擴散半徑 100px       |
| VE-08   | 珍珠奶茶發射                 | 產生三向散射配置                 | 子彈數 3，角度 -15°/0°/+15°          |
| VE-09   | 珍珠奶茶每顆命中             | 產生液體粒子配置                 | 粒子數 50-70/顆，總計 150-210        |
| VE-10   | 豬血糕發射                   | 產生追蹤軌跡配置                 | 曲線半徑 30-60px，週期 0.8s          |
| VE-11   | 豬血糕命中                   | 產生黏稠爆發配置                 | 粒子數 70-100，帶下墜效果            |
| VE-12   | 蚵仔煎發射                   | 產生蓄力特效配置                 | 蓄力時間 0.3s，粒子 100+             |
| VE-13   | 蚵仔煎命中                   | 產生三層爆炸配置                 | 粒子總數 300-500，半徑 150/250/350px |
| VE-14   | 蚵仔煎螢幕震動               | 觸發震動配置                     | 振幅 12px，頻率 50Hz，持續 0.4s      |
| VE-15   | 同時存在多種特效             | 粒子總數控制                     | 總粒子數 < 1000                      |
| VE-16   | Bloom 濾鏡套用               | Game Layer 套用，UI Layer 不套用 | 子彈發光，UI 清晰                    |
| VE-17   | 受擊閃白效果                 | 敵人 sprite 亮度增強             | 持續 0.1-0.2s，亮度 +60% 至 +100%    |
| VE-18   | 粒子物件池回收               | 粒子消失後返回池                 | 記憶體不增長，GC 壓力低              |
| VE-19   | 視覺大小 vs 碰撞箱           | 視覺 > 碰撞箱 1.5-3 倍           | 普通子彈 16px 視覺 / 8px 碰撞        |
| VE-20   | 效能降級（低階裝置）         | 粒子數減少至 30%                 | 仍保持命中爆發，關閉進階效果         |

### 2.10.5.2 Manual Visual Test Checklist（手動視覺測試清單）

**執行方式**: 在瀏覽器中實際遊玩，逐項確認視覺效果符合 SPEC.md § 2.6.3 描述

| 項目 | 測試內容                                             | 通過條件                                         |
| ---- | ---------------------------------------------------- | ------------------------------------------------ |
| MV-01 | 普通子彈拖尾是否清晰可見                             | 白色風切線條明顯，長度 32px                      |
| MV-02 | 普通子彈命中爆發是否有打擊感                         | 白色粒子噴濺，敵人閃白 0.1s，輕微震動            |
| MV-03 | 夜市總匯電弧是否環繞子彈旋轉                         | 8-12 條電弧螺旋運動，金黃色明顯                  |
| MV-04 | 夜市總匯連鎖閃電是否視覺化連結目標                   | 金色閃電鏈清晰連接 5 個敵人，路徑鋸齒狀          |
| MV-05 | 臭豆腐綠色毒霧是否營造「臭味」擴散感                 | 波浪尾跡明顯，粒子隨機漂浮                       |
| MV-06 | 臭豆腐貫穿是否有穿孔視覺                             | 綠色光環從敵人身體擴散，後方爆出臭氣雲           |
| MV-07 | 珍珠奶茶三顆珍珠是否巨大且圓潤                       | 黑色球體漸層，高光反射明顯，體積感強             |
| MV-08 | 珍珠奶茶扇形散射是否覆蓋範圍大                       | 視覺覆蓋畫面 1/4 寬度，三顆軌跡清晰              |
| MV-09 | 豬血糕追蹤軌跡是否呈曲線/螺旋                        | 非直線飛行，曲線路徑視覺化，紅色導引線可見       |
| MV-10 | 豬血糕黏稠拖尾是否有下墜效果                         | 黑紅色粒子緩慢下墜，模擬黏性                     |
| MV-11 | 蚵仔煎蓄力階段是否有能量聚集視覺                     | 紅橙色粒子向玩家收縮，畫面短暫泛紅               |
| MV-12 | 蚵仔煎拋物線是否有重量感                             | 旋轉飛行，重力曲線明顯，巨大體積（128px）        |
| MV-13 | 蚵仔煎爆炸是否分三層且極其華麗                       | 白色閃光 → 紅橙火焰 → 深紅煙霧，粒子 300-500 個  |
| MV-14 | 蚵仔煎螢幕震動是否有極強衝擊感                       | 振幅 12px 明顯，持續 0.4s，分階段震動            |
| MV-15 | 蚵仔煎環境光效是否照亮畫面                           | 紅橙色環境光覆蓋全螢幕，白色光柱從爆炸中心射出   |
| MV-16 | 所有子彈發光（Bloom）是否明顯                        | 金黃/綠色/棕色/深紅光暈清晰可見，脈動效果流暢    |
| MV-17 | 受擊閃白是否增強打擊感                               | 敵人被擊中時短暫閃白，顏色依子彈類型變化         |
| MV-18 | 特效範圍是否比碰撞箱大                               | 視覺範圍明顯大於實際判定，營造浮誇感             |
| MV-19 | 同時多發子彈特效是否流暢                             | 60 FPS 穩定，無卡頓，粒子不閃爍                  |
| MV-20 | 整體視覺是否達到 Vampire Survivors/Brotato 風格水準 | 浮誇、華麗、高辨識度，毀滅性能量感強烈           |

**測試環境**: Chrome 120+, 1920×1080, 60 FPS

**通過條件**: 20 項手動測試至少 18 項通過（90%），且所有「極其華麗」項目（MV-04, MV-13, MV-14, MV-15）必須通過
