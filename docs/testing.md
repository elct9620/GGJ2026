...
| SY-11   | 0    | 0    | 0    | 5    | 蚵仔煎          | 2s            | 基礎傷害 ×8，無消耗，遊戲開始即可使用                                            |
...
| SY-16   | 按 5（蚵仔煎）        | Buff 啟動 2s       | 無消耗，遊戲開始可用  |
| SY-17   | 按 1，食材不足        | 無反應             | UI 提示庫存不足       |
| SY-18   | Buff 期間按 1         | 無反應             | 無法疊加 Buff         |
| SY-19   | Buff 結束後按 5       | Buff 啟動 2s       | 蚵仔煎可重複使用      |
| SY-20   | 選「打折」後按 2      | 消耗豆腐2          | 打折效果永久生效      |
| SY-21   | 選「快吃」後按 2      | 消耗豆腐2          | 快吃效果當場生效      |
| SY-22   | 打折+快吃後按 2       | 消耗豆腐1（最低1） | 兩種效果疊加（最低1） |
