...
| SY-16   | 按 5（蚵仔煎），擊殺數≥20     | 蚵仔煎發射（單發） | 消耗擊殺數 20，擊殺數 -20，**立即發射不啟動 Buff** |
| SY-17   | 按 5（蚵仔煎），擊殺數<20     | 無反應             | UI 提示擊殺數不足                       |
| SY-18   | 按 1，食材不足                | 無反應             | UI 提示庫存不足                         |
| SY-19   | Buff 期間按 1-4               | 無反應             | 無法疊加 Buff（1-4 號）                 |
| SY-19a  | Buff 期間按 5，擊殺數≥20      | 無反應             | 蚵仔煎也受 Buff 堵塞影響                |
| SY-20   | Buff 結束後按 5，擊殺數≥20    | 蚵仔煎發射         | 蚵仔煎可重複使用，再次消耗 20           |
| SY-21   | 選「打折」後按 2              | 消耗豆腐2          | 打折效果永久生效                        |
| SY-22   | 選「快吃」後按 5，擊殺數≥20   | 蚵仔煎發射         | 快吃效果：蚵仔煎傷害 +10%（百分比直接相加） |
| SY-23   | 打折後按 2                    | 消耗豆腐2（最低1） | 打折效果（3→2）                         |
| SY-24   | 擊殺 25 隻 + 選「快吃」+ 按 5 | 蚵仔煎發射         | 消耗仍為 20，快吃效果增加傷害而非減消耗 |
| SY-25   | 擊殺 15 隻 + 按 5             | 無反應             | 消耗需 20，15<20 不可觸發               |

### 2.3.3 Edge Detection Tests

**SPEC § 2.3.3**: 按鍵觸發採用邊緣檢測（Edge Detection），只在按鍵從未按下變為按下時觸發一次。

| Test ID | Input                           | Expected Output                     | Accept Criteria                           |
| ------- | ------------------------------- | ----------------------------------- | ----------------------------------------- |
| SY-30   | 按住按鍵 1 持續 1 秒（60 幀）   | 只觸發 1 次合成                     | 食材只消耗 1 次，Buff 只啟動 1 次         |
| SY-31   | 按住按鍵 5 持續 1 秒（60 幀）   | 只觸發 1 次蚵仔煎                   | 擊殺數只消耗 20，只發射 1 發子彈          |
| SY-32   | 按下按鍵 1 → 放開 → 再按下      | 觸發 2 次合成                       | 兩次邊緣觸發，食材消耗 2 次               |
| SY-33   | 按住 Space 持續 1 秒            | 只發射 1 發子彈                     | 射擊也採用邊緣檢測                        |
| SY-34   | 按下按鍵 1 但食材不足，持續按住 | 只提示 1 次「食材不足」             | 邊緣檢測避免重複提示                      |
| SY-35   | 擊殺 20 隻 → 按住按鍵 5         | 只發射 1 發蚵仔煎，擊殺數變為 0     | 邊緣檢測避免瞬間清空所有擊殺數            |
| SY-36   | 擊殺 40 隻 → 按下放開按鍵 5 兩次| 發射 2 發蚵仔煎，擊殺數變為 0       | 可重複觸發，每次消耗 20                   |
