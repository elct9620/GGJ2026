...
| SY-11   | 0    | 0    | 0    | ≥20    | 1    | 蚵仔煎          | 單發          | 百分比傷害：Boss 10% HP, 菁英 50% HP, 小怪 70% HP，消耗擊殺數 20 |

### 2.3.2 Trigger Special Bullets

| Test ID | Input                         | Expected Output    | Accept Criteria                         |
| ------- | ----------------------------- | ------------------ | --------------------------------------- |
| SY-12   | 按 5（夜市總匯）              | Buff 啟動 2s       | 消耗豆腐1珍珠1米血1                     |
| SY-13   | 按 2（臭豆腐）                | Buff 啟動 2s       | 消耗豆腐3                               |
| SY-14   | 按 3（珍珠奶茶）              | Buff 啟動 2s       | 消耗珍珠3                               |
| SY-15   | 按 4（豬血糕）                | Buff 啟動 2s       | 消耗米血3                               |
| SY-16   | 按 1（蚵仔煎），擊殺數≥20     | 蚵仔煎發射（單發） | 消耗擊殺數 20，擊殺數 -20               |
| SY-17   | 按 1（蚵仔煎），擊殺數<20     | 無反應             | UI 提示擊殺數不足                       |
| SY-18   | 按 5，食材不足                | 無反應             | UI 提示庫存不足                         |
| SY-19   | Buff 期間按 5                 | 無反應             | 無法疊加 Buff                           |
| SY-20   | Buff 結束後按 1，擊殺數≥20    | 蚵仔煎發射         | 蚵仔煎可重複使用，再次消耗 20           |
| SY-21   | 選「打折」後按 2              | 消耗豆腐2          | 打折效果永久生效                        |
| SY-22   | 選「快吃」後按 1，擊殺數≥20   | 蚵仔煎發射         | 快吃效果：蚵仔煎傷害 +10%               |
| SY-23   | 打折後按 2                    | 消耗豆腐2（最低1） | 打折效果（3→2）                         |
| SY-24   | 擊殺 25 隻 + 選「快吃」+ 按 1 | 蚵仔煎發射         | 消耗仍為 20，快吃效果增加傷害而非減消耗 |
| SY-25   | 擊殺 15 隻 + 按 1             | 無反應             | 消耗需 20，15<20 不可觸發               |
...
| EN-27   | 餓瘋鬼     | 5    | 6           | round(6 + (5-5) × 1.0) = 6    |
| EN-28   | 餓瘋鬼     | 10   | 11          | round(6 + (10-5) × 1.0) = 11  |
| EN-29   | 餓瘋鬼     | 15   | 16          | round(6 + (15-5) × 1.0) = 16  |
| EN-30   | 餓幼鬼     | any  | 6           | 固定 6 HP，無成長             |

### 2.7.5 Boss Types

| Test ID | Boss Type | Behavior | Speed | Accept Criteria    |
| ------- | --------- | -------- | ----- | ------------------ |
| EN-31   | 餓死鬼    | 直走     | 0.3   | 只向左移動         |
| EN-32   | 餓瘋鬼    | 斜走     | 0.3   | 斜向移動           |
| EN-33   | 餓幼鬼    | 直走     | 0.7   | 速度比其他 Boss 快 |

### 2.7.6 Booth Interaction

| Test ID | Input               | Expected Output  | Accept Criteria |
| ------- | ------------------- | ---------------- | --------------- |
| EN-34   | 攤位 3/6 + 敵人觸碰 | 攤位 2/6         | 偷取 1 個食材   |
| EN-35   | 敵人偷取後          | 敵人繼續向左移動 | 不停留在攤位    |
| EN-36   | 攤位 0/6 + 敵人觸碰 | 攤位 0/6         | 無食材可偷      |
...
| Test ID | Input                 | Expected Output          | Accept Criteria            |
| ------- | --------------------- | ------------------------ | -------------------------- |
| KC-06   | 遊戲開始，按 1        | 無反應                   | UI 提示擊殺數不足（0/20）  |
| KC-07   | 擊殺 15 隻，按 1      | 無反應                   | UI 提示擊殺數不足（15/20） |
| KC-08   | 擊殺 20 隻，按 1      | 蚵仔煎發射               | 消耗擊殺數 20，剩餘 0      |
| KC-09   | 擊殺 25 隻，按 1      | 蚵仔煎發射               | 消耗擊殺數 20，剩餘 5      |
| KC-10   | 擊殺 40 隻，按 1 兩次 | 第二次蚵仔煎發射         | 可重複消耗，剩餘 0         |
...
| KC-13   | 擊殺 15 隻 + 選「快吃」+ 按 1 | 無反應                | 消耗仍為 20，15<20 不可用 |
| KC-14   | 擊殺 25 隻 + 選「快吃」+ 按 1 | 蚵仔煎發射，傷害 +10% | 消耗 20，傷害增加         |
...
| Test ID | Input                                | Expected Output | Accept Criteria               |
| ------- | ------------------------------------ | --------------- | ----------------------------- |
| KC-18   | 擊殺 20 隻 + 按 1 + 按 Space         | 發射蚵仔煎      | 蚵仔煎效果正確生效            |
| KC-19   | 重新開始遊戲                         | killCount = 0   | 統計重置，擊殺數歸零          |
| KC-20   | 擊殺 19 隻 + 寶箱消滅 1 隻 + 按 1    | 無反應          | 寶箱消滅不計入，仍為 19/20    |
| KC-21   | 結束畫面顯示擊殺數                   | 顯示總擊殺數    | 統計數據正確（含已消耗數量）  |
| KC-22   | 擊殺 25 隻 + 按 1 + 擊殺 3 隻 + 按 1 | 第二次無反應    | 第一次消耗 20，剩 5+3=8，8<20 |
