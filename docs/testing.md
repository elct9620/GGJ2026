...
| SY-22   | 擊殺 7 隻 + 選「快吃」+ 按 5  | Buff 啟動 2s       | 消耗從 10→5，7≥5 可觸發，擊殺數 -5 |
| SY-23   | 擊殺 4 隻 + 選「快吃」+ 按 5  | 無反應             | 消耗從 10→5，4<5 不可觸發      |
...
| UP-18   | 選擇「快吃」               | 蚵仔煎消耗減半（10→5）          | 僅影響蚵仔煎     |
| UP-19   | 多次選「快吃」             | 蚵仔煎消耗持續減半（5→2.5→1.25...，向上取整） | 快吃可重複堆疊   |
