...
| Test ID | Input            | Expected Output                      | Accept Criteria  |
| ------- | ---------------- | ------------------------------------ | ---------------- |
| AU-01   | 進入遊戲場景     | 背景音樂開始播放 (Leisure song.mp3)  | 音樂自動播放     |
| AU-02   | 背景音樂播放中   | 循環播放 (loop = true)               | 音樂不斷播放     |
| AU-03   | 背景音樂播放中   | 音量 = 0.5                           | 音量設定正確     |
| AU-04   | 重複進入遊戲場景 | 不重複播放音樂                       | 避免重複實例     |
| AU-05   | 離開遊戲場景     | 背景音樂停止                         | 音樂正確停止     |
| AU-06   | 音樂檔案不存在   | 記錄錯誤，遊戲繼續                   | 錯誤處理正確     |

### 2.12.2 Button Click Sound

| Test ID | Input          | Expected Output                | Accept Criteria  |
| ------- | -------------- | ------------------------------ | ---------------- |
| AU-07   | 點擊按鈕       | 播放 select03.mp3              | 音效播放         |
| AU-08   | 合成成功       | 播放 select03.mp3              | 音效播放         |
| AU-09   | 升級選擇       | 播放 select03.mp3              | 音效播放         |
| AU-10   | 連續點擊按鈕   | 音效重疊播放                   | 允許重疊         |
| AU-11   | 音效檔案不存在 | 記錄錯誤，遊戲繼續             | 錯誤處理正確     |

### 2.12.3 Player Shoot Sound

| Test ID | Input          | Expected Output                | Accept Criteria  |
| ------- | -------------- | ------------------------------ | ---------------- |
| AU-12   | Space 發射子彈 | 播放 shoot5.mp3                | 音效播放         |
| AU-13   | 連續射擊       | 音效重疊播放                   | 允許重疊         |
| AU-14   | 重裝期間       | 不觸發音效                     | 無音效播放       |
| AU-15   | 彈藥歸零射擊   | 不觸發音效                     | 無音效播放       |
| AU-16   | 音效檔案不存在 | 記錄錯誤，遊戲繼續             | 錯誤處理正確     |

### 2.12.4 Enemy Hit Sound

| Test ID | Input          | Expected Output                | Accept Criteria  |
| ------- | -------------- | ------------------------------ | ---------------- |
| AU-17   | 子彈擊中敵人   | 播放 short_punch1.mp3          | 音效播放         |
| AU-18   | 連續擊中敵人   | 音效重疊播放                   | 允許重疊         |
| AU-19   | 貫穿擊中多敵人 | 多次播放音效                   | 每次擊中觸發     |
| AU-20   | 連鎖擊中多敵人 | 多次播放音效                   | 每次擊中觸發     |
| AU-21   | 敵人到達底線   | 不觸發音效                     | 無音效播放       |
| AU-22   | 音效檔案不存在 | 記錄錯誤，遊戲繼續             | 錯誤處理正確     |

### 2.12.5 Event Integration

| Test ID | Event Type         | Expected Audio Action          | Accept Criteria  |
| ------- | ------------------ | ------------------------------ | ---------------- |
| AU-23   | PlayerShoot        | 播放射擊音效                   | 事件正確訂閱     |
| AU-24   | EnemyHit           | 播放受擊音效                   | 事件正確訂閱     |
| AU-25   | ButtonClick        | 播放按鈕音效                   | 事件正確訂閱     |
| AU-26   | SynthesisTriggered | 播放按鈕音效                   | 事件正確訂閱     |
| AU-27   | UpgradeSelected    | 播放按鈕音效                   | 事件正確訂閱     |
| AU-28   | GameSceneEnter     | 開始背景音樂                   | 事件正確訂閱     |
...
| IT-12   | 音效系統 | 事件佇列 | 訂閱射擊事件       | 射擊時播放音效           | 事件驅動正確    |
| IT-13   | 音效系統 | 戰鬥系統 | 敵人受擊觸發音效   | 每次擊中播放音效         | 系統整合正常    |
| IT-14   | 音效系統 | 合成系統 | 合成成功觸發音效   | 合成時播放按鈕音效       | 系統整合正常    |
| IT-15   | 音效系統 | 升級系統 | 升級選擇觸發音效   | 升級時播放按鈕音效       | 系統整合正常    |
| IT-16   | 音效系統 | 遊戲場景 | 場景進入開始音樂   | 進入場景背景音樂播放     | 系統整合正常    |
