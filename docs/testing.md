...
| SY-11   | 0    | 0    | 0    | 5    | 蚵仔煎          | 2s            | 基礎傷害 ×8，無消耗，需累積擊殺 10 隻解鎖                                        |
...
| SY-16   | 按 5（蚵仔煎），已解鎖  | Buff 啟動 2s       | 無消耗，擊殺數 ≥10             |
| SY-17   | 按 5（蚵仔煎），未解鎖  | 無反應             | UI 提示未解鎖（擊殺數 <10）    |
| SY-18   | 按 1，食材不足          | 無反應             | UI 提示庫存不足                |
| SY-19   | Buff 期間按 1           | 無反應             | 無法疊加 Buff                  |
| SY-20   | Buff 結束後按 5，已解鎖 | Buff 啟動 2s       | 蚵仔煎可重複使用               |
| SY-21   | 選「打折」後按 2        | 消耗豆腐2          | 打折效果永久生效               |
| SY-22   | 選「快吃」後按 2        | 消耗豆腐2          | 快吃效果當場生效               |
| SY-23   | 打折+快吃後按 2         | 消耗豆腐1（最低1） | 兩種效果疊加（最低1）          |
| SY-24   | 擊殺 7 隻 + 選「快吃」  | 蚵仔煎立即解鎖     | 門檻從 10→5，7≥5 自動解鎖      |
| SY-25   | 擊殺 4 隻 + 選「快吃」  | 蚵仔煎未解鎖       | 門檻從 10→5，4<5 仍未解鎖      |
...
## 2.10 Kill Counter and Oyster Omelet Unlock Tests
...
### 2.10.2 Oyster Omelet Unlock

| Test ID | Input                          | Expected Output            | Accept Criteria                     |
| ------- | ------------------------------ | -------------------------- | ----------------------------------- |
| KC-06   | 遊戲開始，按 5                 | 無反應                     | UI 提示未解鎖（0/10）               |
| KC-07   | 擊殺 5 隻，按 5                | 無反應                     | UI 提示未解鎖（5/10）               |
| KC-08   | 擊殺 10 隻，按 5               | Buff 啟動 2s               | 蚵仔煎解鎖（10/10）                 |
| KC-09   | 擊殺 15 隻，按 5               | Buff 啟動 2s               | 已解鎖狀態持續（15/10）             |
| KC-10   | UI 顯示解鎖進度                | 顯示 X/10                  | 實時更新擊殺進度                    |

### 2.10.3 Quick Eat Effect on Oyster Omelet

| Test ID | Input                              | Expected Output        | Accept Criteria                        |
| ------- | ---------------------------------- | ---------------------- | -------------------------------------- |
| KC-11   | 選擇「快吃」升級                   | 解鎖門檻變為 5         | UI 顯示 X/5                            |
| KC-12   | 擊殺 3 隻 + 選「快吃」             | 蚵仔煎未解鎖           | 門檻 10→5，3<5 未解鎖（3/5）           |
| KC-13   | 擊殺 7 隻 + 選「快吃」             | 蚵仔煎立即解鎖         | 門檻 10→5，7≥5 自動解鎖（7/5）         |
| KC-14   | 選「快吃」+ 擊殺 5 隻，按 5        | Buff 啟動 2s           | 累積速度 ×2，5 隻即達標（5/5）         |
| KC-15   | 快吃升級後 UI 顯示                 | 顯示 X/5 而非 X/10     | 解鎖門檻正確更新                       |
| KC-16   | 多次選「快吃」升級                 | 門檻仍為 5             | 快吃不疊加（門檻固定 5，不會變 2.5）   |

### 2.10.4 Integration Tests

| Test ID | Input                                  | Expected Output              | Accept Criteria                  |
| ------- | -------------------------------------- | ---------------------------- | -------------------------------- |
| KC-17   | 擊殺 10 隻 + 按 5 + 按 Space           | 發射高傷害子彈               | 蚵仔煎 Buff 正確生效             |
| KC-18   | 重新開始遊戲                           | killCount = 0，蚵仔煎鎖定    | 統計重置，解鎖狀態重置           |
| KC-19   | 擊殺 9 隻 + 寶箱消滅 1 隻 + 按 5       | 無反應                       | 寶箱消滅不計入，仍為 9/10        |
| KC-20   | 結束畫面顯示擊殺數                     | 顯示總擊殺數                 | 統計數據正確                     |
